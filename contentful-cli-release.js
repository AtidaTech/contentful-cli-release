...
;(async function main() {
    try {
        ...
        const spaceSingleton = await getSpace(contentfulManagement, contentfulLib, parsedArguments)

        switch (parsedArguments.chosenAction) {
            case 1:
                await duplicateMaster(contentfulManagement, contentfulLib, spaceSingleton, parsedArguments)
                break
            case 2:
                await syncScheduledActions(contentfulManagement, contentfulLib, spaceSingleton, parsedArguments)
                break;
        //     case 3:
...
async function parseArguments(rootFolder, envValues) {
    ...
    const {
        ...
        'sync-db': syncPath =
            envValues?.CMS_RELEASE_DB ??
            RELEASE_DB,
        'config-file': configPath =
            envValues?.CMS_RELEASE_CONFIG ??
            RELEASE_CONFIG,
        'main-environment': mainEnvironment =
            envValues?.CMS_RELEASE_ENVIRONMENT_MAIN ??
            RELEASE_ENVIRONMENT_MAIN,
        'protected-environments': protectedEnvironments =
            envValues?.CMS_RELEASE_ENVIRONMENT_PROTECTED ??
            RELEASE_ENVIRONMENT_PROTECTED,
        'release-regex': releaseRegularExpression =
            envValues?.CMS_RELEASE_ENVIRONMENT_REGEX ??
...
async function getEnvFromArgs(parsedArgs, chosenAction) {
    if (!((Boolean(parsedArgs.from) || Boolean(parsedArgs.to)) ^ Boolean(parsedArgs['environment-id']))) {
        console.error(
 ...
 */
async function duplicateMaster(contentfulManagement, contentfulLib, spaceSingleton, parsedArguments)
{
    ...
    if (duplicateEnvironment) {
        ...
        let timerId = setInterval(() => {
            duplicateEnvironment
                ...
                .catch(e => {
                    console.log(
                        '%%/DEBUG: Waiting to retrieve the newly created environment ' + parsedArguments?.environmentId
                    )
                })
        }, 1000)
    }

    console.error(`@@/ERROR: There was an error duplicating the environment ${parsedArguments?.environmentId}`)
    console.error('@@/ERROR: Please check the input parameters and investigate the previous error messages for more details')
}
 ...
 * @returns {Promise<void>}
 */
async function syncScheduledActions(contentfulManagement, contentfulLib, spaceSingleton, parsedArguments)
{
    await validateEnvironments(
        contentfulManagement,
        contentfulLib,
...
async function validateEnvironments(
    ...
    if (parsedValues.environmentId === '' || excludedEnvironments.includes(parsedValues.environmentId)) {
        console.error(
    ...
    const regex = new RegExp(
        parsedValues.releaseRegularExpression,
        'g'
    )
